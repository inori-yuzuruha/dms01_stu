...
public class MainFrame_temp extends JFrame {
	...
	public MainFrame_temp() {
		...
		new UpdataTableThread().start();//线程的开始，在面板存在的时候就要开始线程
	}
	...
	private void initTransGatherGUI() {
		...
		DefaultComboBoxModel<String> comboModel = new DefaultComboBoxModel<String>(tranStatus);
	...
	private class MatchTransListener implements ActionListener {
		// 数据匹配的事件处理方法
		public void actionPerformed(ActionEvent e) {
			TransportAnalyse logAn = new TransportAnalyse(transList);
			logAn.doFilter();
			matchedTrans = logAn.matchData();
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {		
			...
			try{
			   ...
			   JOptionPane.showMessageDialog(null, "日志信息保存成功！", "提示",
					JOptionPane.INFORMATION_MESSAGE);
			}catch(Exception e1) {
				e1.printStackTrace();
				JOptionPane.showMessageDialog(null, "日志信息保存失败！", "警告",
			...
			try{
			   ...
			   JOptionPane.showMessageDialog(null, "物流信息保存成功！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
			}catch(Exception e1) {
				e1.printStackTrace();
				JOptionPane.showMessageDialog(null, "物流信息保存失败！", "警告",
						JOptionPane.ERROR_MESSAGE);
			}
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if(matchedLogs != null && matchedLogs.size() > 0) {
					...
					matchedLogs.clear();
					logList.clear();
					JOptionPane.showMessageDialog(null, "匹配的日志数据已发送到服务器！","提示",	
							JOptionPane.INFORMATION_MESSAGE);
				}else {
				...
				if(matchedTrans != null && matchedTrans.size() > 0) {
					...
					Socket tranSocket = new Socket(serverIP,6666);
					ObjectOutputStream tranOutputStream = new ObjectOutputStream(
							tranSocket.getOutputStream());
					tranOutputStream.writeObject(matchedTrans);
					tranOutputStream.flush();
					tranOutputStream.close();
					matchedTrans.clear();
					transList.clear();
					JOptionPane.showMessageDialog(null, "匹配的物流数据已发送到服务器！","提示",	
							JOptionPane.INFORMATION_MESSAGE);
				}else {
	...
	//线程类，每隔一分钟显示数据表格中的数据与数据库保持同步
	private class UpdataTableThread extends Thread{
	    //重写run方法
		public void run() {
			while(true) {
				//移除所有选项卡
